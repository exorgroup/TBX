    ...
    jQuery(document).ready(function($) {
      ...
      $('#crudTable').on( 'draw.dt',   function () {
        ...
        if (! crud.table.responsive.hasHidden()) {
         ...
         // Apply custom styling to action columns
         applyActionColumnStyles();
      ...
      // Function to apply styling to action column
      function applyActionColumnStyles() {
        let actionColumn = document.querySelector('th[data-action-column="true"]');
        
        if (actionColumn) {
          // Apply custom width if defined
          if (actionColumn.dataset.columnWidth) {
            actionColumn.style.width = actionColumn.dataset.columnWidth;
          }
          
          // Apply header alignment if defined
          if (actionColumn.dataset.headerAlign) {
            actionColumn.style.textAlign = actionColumn.dataset.headerAlign;
          }
          
          // Apply cell alignment to all cells in the actions column
          if (actionColumn.dataset.cellAlign) {
            let columnIndex = $('#crudTable th').index(actionColumn);
            
            $('#crudTable tbody tr').each(function() {
              let actionCell = $(this).find('td').eq(columnIndex);
              actionCell.css('text-align', actionColumn.dataset.cellAlign);
              
              // Also align the button group inside the cell
              let buttonGroup = actionCell.find('.btn-group');
              if (buttonGroup.length) {
                if (actionColumn.dataset.cellAlign === 'center') {
                  buttonGroup.css({
                    'display': 'flex',
                    'justify-content': 'center'
                  });
                } else if (actionColumn.dataset.cellAlign === 'right') {
                  buttonGroup.css({
                    'display': 'flex',
