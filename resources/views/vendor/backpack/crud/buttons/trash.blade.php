@if ($crud->hasAccess('trash', $entry) && $entry->trashed() === false)
    ...
    data-route="{{ url($crud->route.'/'.$entry->getKey().'/trash') }}" class="btn btn-sm btn-outline-secondary btn-pill" data-button-type="trash"><i class="la la-trash"></i> <span>{{ trans('backpack/pro::trash.trash') }}</span></a>
@endif
